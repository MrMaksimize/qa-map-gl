      ...
      .pipe(
        stylemod({
          moduleId: function(file) {
            return (
              path.basename(file.path, path.extname(file.path)) + "-styles"
            );
          }
        })
      )
...
gulp.task("dist:js", ["dist:clean"], function() {
  ...
  bundler
    .pipe(source("qa-map-gl-bundle.js"))
    .pipe(buffer())
  ...
  globby(["./qa-*.es6.js"]).then(entries => {
    ...
    entries = entries.reduce(
      (accum, entry) => {
        if (/behavior\-element\.es6\.js$/.test(entry)) {
          accum[0] = entry;
        } else if (/behavior\-layer\.es6\.js$/.test(entry)) {
          accum[1] = entry;
        } else {
          accum = accum.concat([entry]);
        }
        return accum;
      },
      [null, null]
...
gulp.task("dist:makeloader", ["dist:html"], function(cb) {
  fs.writeFile(
    "dist/qa-map-gl-bundle-loader.html",
    '<script src="qa-map-gl-bundle.js"></script>',
...
gulp.task("serve:dist", function() {
  ...
  gulp.watch(["sass/*.scss"], ["sass"]);
  gulp
    .watch(["css/*-styles.html", "./*.html", `./*.js`, "demo/*.html"])
    .on("change", browserSync.reload);
});
