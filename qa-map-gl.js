...
import { QaElementMixin } from './mixins/element.js';
import './styles/qa-map-gl-mapbox-styles.js';
import './styles/qa-map-gl-geocoder-styles.js';

class QaMapGl extends QaElementMixin(PolymerElement) {
  static get template() {
    return html`
  ...
  connectedCallback() {
    ...
    this.addEventListener("qa-map-gl-element-ready-to-add", this.shouldAddInst.bind(this));
  ...
  createInst(options) {
    window.mapboxgl.accessToken = this.mglToken;
    options.container = this.shadowRoot.querySelector(
      options.container
    );
    const mapInst = new window.mapboxgl.Map(options);
  ...
  _updateMapView() {
    ...
    this._debounceUpdateMapView = Debouncer.debounce(this._debounceUpdateMapView,
      microTask, () => {
        const {
          lng,
          lat
        } = this.elementInst.getCenter();
  ...
  latLngIsValid(lat, lng) {
    ...
    console.error(`qa-map-gl CONFIGURATION ERROR:
  ...
  _handleMapLoaded(e) {
    if (this.canAddInst()) {
      this._debounceFireLoadEvents = Debouncer.debounce(this._debounceFireLoadEvents,
        microTask, () => {
  ...
  _handleMapMove() {
    ...
    this._debounceHandleMapMove = Debouncer.debounce(this._debounceHandleMapMove,
      timeOut.after(1000), () => {
...
window.customElements.define('qa-map-gl', QaMapGl);