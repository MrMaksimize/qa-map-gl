...
import { QaSourceMixin } from './mixins/source.js';

class QaMapGlSourceGeojson extends QaSourceMixin(QaElementMixin(PolymerElement)) {
  static get template() {
    return html`
      <style>
        :host {
  ...
  static get observers() {
    return ["shouldUpdateInstComplex(data.*)"];
  }
 ...
 */
  ...
  shouldUpdateInstComplex(data) {
    this._debounceShouldUpdateInst = Debouncer.debounce(this._debounceShouldUpdateInst,
      timeOut.after(250), () => super.shouldUpdateInst(parent));
  }

  // extends the layer `addInst` method to harvest and fire events
  addInst(parent) {
    super.addInst(parent);
  }
  ...
  getInstOptions() {
    ...
    const options = super.getInstOptions();
...
window.customElements.define('qa-map-gl-source-geojson', QaMapGlSourceGeojson);